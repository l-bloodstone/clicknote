...
func main() {
	...
	if _, err := db.Exec("create table if not exists note (noteId text primary key, passhash text, data text)"); err != nil {
		panic(err)
	}
	...
	app.POST("/create_note", func(c *gin.Context) {
		noteR := NoteRequest{}
		if err := c.ShouldBindBodyWithJSON(&noteR); err != nil {
			c.JSON(501, gin.H{"status": "Failed! Invalid Info."})
			return
		}

		_, err := db.Exec("insert into note values(?, ?, ?)", noteR.NodeId, noteR.PassHash, "")
		if err != nil {
